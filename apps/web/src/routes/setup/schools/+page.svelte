...
<Card.Root>
	<Card.Header>
		...
		<Card.Description>
			Upload a CSV with columns <code>parent_code,level,code,label</code>. Parents must appear before children.
		</Card.Description>
	</Card.Header>
	<Card.Content>
		<details class="mb-4 text-sm">
			<summary class="cursor-pointer text-muted-foreground hover:text-foreground">Show example</summary>
...
S,school,S-001,Adams Middle</pre>
		...
		<form method="POST" action="?/import" class="space-y-4">
			<Textarea name="csv" rows={10} required placeholder="parent_code,level,code,label..." class="font-mono text-sm" />
